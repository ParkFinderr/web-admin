...
function EditGedungModal({ parking, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(used) || used < 0 || used > parking.totalSlots) {
      setErr(`Slot terisi harus antara 0 – ${parking.totalSlots}`); return
    }
    setLoading(true)
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', zIndex:999, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }} onClick={onClose}>
      <div style={{ background:'var(--bg-card)', border:'1px solid var(--border2)', borderRadius:'var(--radius-lg)', width:'100%', maxWidth:460, overflow:'hidden', animation:'fadeUp .35s ease both' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding:'18px 22px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div>
              <div style={{ fontSize:15, fontWeight:800, color:'var(--text)' }}>Edit Info Gedung</div>
              <div style={{ fontSize:11, color:'var(--text3)' }}>{parking.name}</div>
            </div>
          </div>
        ...
        {/* Staff notice */}
        <div style={{ padding:'9px 22px', background:'rgba(245,158,11,0.06)', borderBottom:'1px solid var(--border)', fontSize:12, color:'var(--orange)' }}>
        ...
        {success ? (
          <div style={{ padding:'40px 22px', textAlign:'center' }}>
            ...
            <h3 style={{ fontSize:17, fontWeight:800, color:'var(--text)', marginBottom:8 }}>Berhasil Diperbarui!</h3>
            <p style={{ fontSize:13, color:'var(--text3)', marginBottom:20 }}>Data gedung telah berhasil disimpan.</p>
          ...
          <form onSubmit={handleSubmit} style={{ padding:22 }}>
            <div style={{ marginBottom:14 }}>
              <label style={{ display:'block', fontSize:13, fontWeight:600, color:'var(--text2)', marginBottom:6 }}>Alamat Gedung</label>
              <textarea className="input" rows={2} value={form.address} onChange={e => { setForm(f=>({...f,address:e.target.value})); setErr('') }}
                style={{ resize:'vertical', minHeight:60 }} />
            </div>
            <div style={{ marginBottom:14 }}>
              <label style={{ display:'block', fontSize:13, fontWeight:600, color:'var(--text2)', marginBottom:6 }}>Catatan Operasional</label>
              <textarea className="input" rows={2} placeholder="Contoh: Lantai B1 sedang perbaikan, gunakan L1..." value={form.catatan} onChange={e => setForm(f=>({...f,catatan:e.target.value}))}
                style={{ resize:'vertical', minHeight:60 }} />
            </div>
            <div style={{ marginBottom: err ? 8 : 20 }}>
              <label style={{ display:'block', fontSize:13, fontWeight:600, color:'var(--text2)', marginBottom:6 }}>
                Slot Terisi Saat Ini <span style={{ fontWeight:400, color:'var(--text3)' }}>(maks. {parking.totalSlots})</span>
              </label>
              <input className="input" type="number" min="0" max={parking.totalSlots} value={form.usedSlots}
                onChange={e => { setForm(f=>({...f,usedSlots:e.target.value})); setErr('') }}
                style={{ borderColor: err ? 'var(--red)':undefined }} />
            </div>
            {err && <div style={{ fontSize:12, color:'var(--red)', marginBottom:14 }}>⚠ {err}</div>}
...
export default function StaffDashboard() {
  ...
  const [localParking, setLocalParking] = useState(
    () => baseParkings.find(p => p.id === user?.parkingId) || baseParkings[0]
  )
  ...
  return (
    <div>
      ...
      {parking.catatan && (
        <div style={{ background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.25)', borderRadius:'var(--radius)', padding:'10px 16px', marginBottom:18, fontSize:13, color:'var(--orange)', display:'flex', gap:8 }}>
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <div style={{ textAlign:'center', padding:'10px 0 20px' }}>
              <div style={{ fontSize:60, fontWeight:900, color:occupancyColor, lineHeight:1 }}>
                {parking.occupancy}%
              </div>
              <div style={{ fontSize:13, color:'var(--text3)', marginTop:4 }}>Tingkat Penggunaan</div>
              <div style={{ marginTop:16, maxWidth:320, margin:'16px auto 0' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'var(--text3)', marginBottom:6 }}>
                  <span>0%</span><span>50%</span><span>100%</span>
                </div>
                <div className="progress" style={{ height:14, borderRadius:99 }}>
                  <div className="progress-bar" style={{ width:parking.occupancy+'%', background:occupancyColor, borderRadius:99, transition:'width .8s ease' }} />
                </div>
              </div>
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginTop:8 }}>
              {[
                { label:'Total Slot', value: parking.totalSlots, color:'var(--text)', bg:'var(--bg-hover)' },
                { label:'Terisi', value: parking.usedSlots, color:'var(--red)', bg:'rgba(239,68,68,0.08)' },
                { label:'Tersedia', value: available, color:'var(--green)', bg:'rgba(34,197,94,0.08)' },
                { label:'Lantai Aktif', value: parking.floors?.length || 0, color:'var(--accent)', bg:'var(--accent-glow)' },
              ].map((s, i) => (
                <div key={i} style={{ background:s.bg, border:'1px solid var(--border)', borderRadius:12, padding:'12px 16px', textAlign:'center' }}>
            ...
            {/* Lantai */}
            {parking.floors?.length > 0 && (
              <div style={{ marginTop:16, paddingTop:14, borderTop:'1px solid var(--border)' }}>
                <div style={{ fontSize:12, fontWeight:700, color:'var(--text2)', marginBottom:8 }}>Lantai Tersedia:</div>
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {parking.floors.map(f => (
                    <span key={f} style={{ fontSize:12, background:'var(--bg-hover)', border:'1px solid var(--border)', borderRadius:8, padding:'4px 10px', color:'var(--text3)', fontWeight:600 }}>{f}</span>
                  ))}
                </div>
              </div>
            )}
      ...
      <div className="card">
        <div className="card-header">
          <div style={{ display:'flex', gap:0, background:'var(--bg-base)', border:'1px solid var(--border)', borderRadius:10, overflow:'hidden' }}>
            {[['scan','📡 Log Scan QR'], ['booking','📋 Booking'], ['swap','🔄 Tukar Slot']].map(([t, l]) => (
              <button key={t} onClick={() => { setTab(t); setSearch('') }} className={`filter-tab ${tab === t ? 'active' : ''}`}>
                {l}
              </button>
            ))}
          </div>
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            <div className="input-icon-wrap" style={{ minWidth:200 }}>
              ...
              <input className="input" placeholder="Cari..." value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft:34, fontSize:13 }} />
            </div>
          </div>
        </div>

        {/* Live indicator */}
        <div style={{ padding:'10px 20px', background:'rgba(34,197,94,0.04)', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:8, fontSize:12, color:'var(--green)' }}>
            ...
            <table className="data-table">
              ...
              <tbody>
                {filteredScans.length === 0 ? (
                  <tr><td colSpan={6}><div className="empty-state"><div className="empty-icon">📡</div><div>Belum ada aktivitas scan</div></div></td></tr>
                ) : filteredScans.map(s => (
                  <tr key={s.id}>
                    ...
                    <td><span style={{ fontFamily:'monospace', fontWeight:700, fontSize:13 }}>{s.plate}</span></td>
            ...
            <table className="data-table">
              ...
              <tbody>
                {filteredBookings.length === 0 ? (
                  <tr><td colSpan={6}><div className="empty-state"><div className="empty-icon">📋</div><div>Belum ada booking untuk gedung ini</div></div></td></tr>
                  ...
                  <tr key={b.id}>
                    ...
                    <td><span style={{ fontFamily:'monospace', fontWeight:700, fontSize:13 }}>{b.plate}</span></td>
                    <td style={{ fontSize:13 }}>{b.floor} / {b.slot}</td>
            ...
            <table className="data-table">
              ...
              <tbody>
                {filteredSwaps.length === 0 ? (
                  <tr><td colSpan={6}><div className="empty-state"><div className="empty-icon">🔄</div><div>Belum ada tukar slot untuk gedung ini</div></div></td></tr>
                ) : filteredSwaps.map(s => (
                  <tr key={s.id}>
                    <td style={{ fontSize:12, fontFamily:'monospace', color:'var(--text3)' }}>{s.id}</td>
                    <td>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        <div className="avatar avatar-sm">{s.userName[0]}</div>
                        <div>
